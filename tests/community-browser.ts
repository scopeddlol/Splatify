...
export async function runCommunityBrowser(
  screenshots: boolean,
  extension?: (context: any) => Promise<void>,
) {
  const {
  ...
  assert.ok(
    BROWSER_DATABASE_URL && PLAYWRIGHT_MODULE && BROWSER_EXECUTABLE,
    "Set BROWSER_DATABASE_URL, PLAYWRIGHT_MODULE, BROWSER_EXECUTABLE",
  );
  if (screenshots)
    assert.ok(
      UI_SCREENSHOT_DIR,
      "Set UI_SCREENSHOT_DIR to an existing directory",
    );
  ...
  const prefix = `v3-smoke-${randomUUID()}`;
  ...
  const submit = async (page: any, label: string) => {
    await Promise.all([
      page.waitForResponse(
        (response: any) => response.request().method() === "POST",
      ),
    ...
    await page.waitForLoadState("networkidle");
    assert.ok(
      !new URL(page.url()).searchParams.has("error"),
  ...
  const seedUser = async (role: string) => {
    ...
    return (
      await db.query(
        ...
        [`${prefix}-${role}`, email, await hashPassword(password)],
      )
    ).rows[0];
  };
  ...
  try {
    ...
    await check("signed-in create public day and own overview", async () => {
      ...
      event = (
        await db.query("SELECT * FROM events WHERE title=$1 AND owner_id=$2", [
          prefix,
          ownerUser.id,
        ])
      ).rows[0];
    ...
    for (const state of ["TX", "Texas"])
      await check(`anonymous Explore search Austin/${state}`, async () => {
    ...
    await check("password toggle does not submit", async () => {
      ...
      const toggle = anonymous.getByRole("button", {
        name: "Show password",
        exact: true,
      });
    ...
    await check(
      "custom invitation headline persists independently of day settings",
      async () => {
        await goto(owner, `${day}/settings`);
        await owner
          .locator('[name="invitationHeading"]')
          .fill(`${prefix} welcome`);
        await owner
          .locator('[name="invitationMessage"]')
          .fill("Bring your crew for the smoke test.");
        await submit(owner, "Save invitation welcome");
        await goto(owner, `${day}/settings`);
        await submit(owner, "Save day details");
        await goto(anonymous, invite);
        assert.equal(
          await anonymous.locator("h1").innerText(),
          `${prefix} welcome`,
        );
        assert.equal(
          (
            await db.query(
              "SELECT invitation_heading FROM events WHERE id=$1",
              [event.id],
            )
          ).rows[0].invitation_heading,
          `${prefix} welcome`,
        );
      },
    );
    ...
    const image = {
      name: "v2-smoke.png",
      mimeType: "image/png",
      buffer: await requireModule("sharp")({
        create: { width: 120, height: 120, channels: 3, background: "#d5fb51" },
      })
        .png()
        .toBuffer(),
    };
    await check(
      "profile display name, private real name, marker and avatar persist",
      async () => {
        await goto(member, "/profile");
        await member.locator('[name="displayName"]').fill(`${prefix}-display`);
        await member.getByText("Private details", { exact: true }).click();
        await member
          .locator('[name="realName"]')
          .fill(`${prefix}-PRIVATE-REAL`);
        ...
        await goto(member, "/profile");
        assert.equal(
          await member.locator('[name="displayName"]').inputValue(),
          `${prefix}-display`,
        );
        assert.equal(
          await member.locator('[name="marker"]').inputValue(),
          "electric",
        );
        const avatar = member.locator('img[src^="/media/"]').first();
        await avatar.waitFor();
        await avatar.evaluate((img: HTMLImageElement) => img.decode());
        const saved = (
          await db.query("SELECT * FROM users WHERE id=$1", [memberUser.id])
        ).rows[0];
    ...
    await check(
      "profile defaults prefill RSVP; signed-in membership established",
      async () => {
        await goto(member, invite);
        assert.equal(
          await member.locator('#rsvp [name="name"]').inputValue(),
          `${prefix}-display`,
        );
        assert.equal(
          await member.locator('#rsvp [name="marker"]').inputValue(),
          "electric",
        );
    ...
    await check(
      "accepted member stable day across fresh login without guest cookie",
      async () => {
        const fresh = await newPage();
        await login(fresh, memberUser.email);
        assert.equal(
          (await fresh.context().cookies()).filter((c: any) =>
            c.name.startsWith("splatify_guest_"),
          ).length,
          0,
        );
        ...
        assert.ok(
          (await fresh.locator(".day-roster").innerText()).includes(
            `${prefix}-display`,
          ),
        );
        assert.ok(
          !(await fresh.locator("body").textContent()).includes(
            `${prefix}-PRIVATE-REAL`,
          ),
        );
    ...
    await check("signed-in accepted member posts message", async () => {
      ...
      await submit(member, "Send");
      assert.ok(
        (await member.locator(".day-message-list").innerText()).includes(
          `${prefix}-private-message`,
        ),
      );
    });
    await check(
      "public anonymous DOM excludes roster, messages and real name",
      async () => {
        for (const route of [
          day,
          `${day}/players`,
          ...
          for (const secret of [
            ...
            assert.ok(!dom.includes(secret), `${route} leaked ${secret}`);
    ...
    await check(
      "signup invite next survives recovery acknowledgement",
      async () => {
    ...
    await check(
      "anonymous RSVP private edit link claims on another device",
      async () => {
        await goto(anonymous, invite);
        await anonymous.locator('#rsvp [name="name"]').fill(`${prefix}-guest`);
        await submit(anonymous, "Send my RSVP");
        const cookie = (await anonymous.context().cookies()).find(
          (c: any) => c.name === `splatify_guest_${event.id}`,
        );
        ...
        assert.equal(
          await fresh.locator('#rsvp [name="name"]').inputValue(),
          `${prefix}-guest`,
        );
    ...
    await check(
      "linked guest edit token requires account sign-in",
      async () => {
        ...
        assert.match(
          new URL(fresh.url()).searchParams.get("error")!,
          /invalid or expired/,
        );
        assert.equal(
          (await fresh.context().cookies()).filter((c: any) =>
    ...
    await check(
      "disabled member invites makes requests pending; owner approval grants roster",
      async () => {
        await goto(owner, `${day}/settings`);
        await owner
          .locator("label.settings-toggle")
          .filter({ has: owner.locator('[name="memberInvitesEnabled"]') })
          .click();
        assert.equal(
          await owner.locator('[name="memberInvitesEnabled"]').isChecked(),
          false,
        );
        await submit(owner, "Save day details");
        ...
        assert.equal(
          (
            await db.query(
              "SELECT approval FROM guests WHERE event_id=$1 AND user_id=$2",
              [event.id, pendingUser.id],
            )
          ).rows[0].approval,
          "pending",
        );
        ...
        assert.ok(
          !(await pending.content()).includes(`${prefix}-private-message`),
        );
        await goto(owner, `${day}/players`);
        await owner
          .locator(".day-approval article")
          .filter({ hasText: pendingUser.name })
    ...
    await check(
      "captain picks unassigned and releases own player",
      async () => {
        ...
        await owner
          ...
          .filter({ hasText: "Smoke team" })
          .getByText("Edit team", { exact: true })
          .click();
        const edit = owner.locator('form:has([name="captainUserId"])');
        await edit
          .locator('[name="captainUserId"]')
          .selectOption(memberUser.id);
        await edit
          .getByRole("button", { name: "Save team", exact: true })
          .click();
        await owner.waitForURL(/success=/);
        await goto(member, `${day}/teams`);
        await member.getByText("Assign players", { exact: true }).click();
        await member
          .locator(".team-picker-row")
          .filter({ hasText: `${prefix}-guest` })
          .locator("input")
          .check();
        await submit(member, "Assign selected");
        await goto(member, `${day}/teams`);
        assert.ok(
          (
            await db.query(
              "SELECT team_id FROM guests WHERE event_id=$1 AND name=$2",
              [event.id, `${prefix}-guest`],
            )
          ).rows[0].team_id,
        );
        await member.getByText("Assign players", { exact: true }).click();
        await member.locator('.team-manager [name="teamId"]').selectOption("");
        await member
          .locator(".team-picker-row")
          .filter({ hasText: `${prefix}-guest` })
          .locator("input")
    ...
    await check(
      "co-organizer delegated settings and planning, no appointment or deletion",
      async () => {
        await goto(owner, `${day}/settings`);
        await owner
        ...
        assert.ok(
          await member
            .getByRole("button", { name: "Save day details", exact: true })
            .count(),
        );
        assert.equal(
          await member
        ...
        assert.ok(
          await member
            .getByRole("heading", { name: "Smoke briefing", exact: true })
            .count(),
        );
        await goto(member, `${day}/gear`);
        await member.getByText("Add gear or a cost", { exact: true }).click();
        ...
        assert.ok(
          await member
            .getByRole("heading", { name: "Smoke rental mask", exact: true })
            .count(),
        );
      },
    );
    await check(
      "cover and invitation uploads render and enforce private media access",
      async () => {
        await goto(owner, `${day}/settings`);
        ...
        await member
          .locator(`img[src="/media/${saved.cover_id}"]`)
          .evaluate((img: HTMLImageElement) => img.decode());
        const fresh = await newPage();
        await goto(fresh, invite);
        await fresh
          .locator(`img[src*="${saved.invitation_cover_id}"]`)
          .evaluate((img: HTMLImageElement) => img.decode());
        assert.equal(
          (await fresh.request.get(`${base}/media/${saved.cover_id}`)).status(),
          200,
        );
        assert.equal(
          (
            await fresh.request.get(
              `${base}/media/${saved.invitation_cover_id}`,
            )
          ).status(),
          200,
        );
        ...
        assert.equal(
          (await fresh.request.get(`${base}/media/${avatarId}`)).status(),
          404,
        );
        assert.equal(
          (
            await member.request.get(`${base}/media/${saved.cover_id}`)
          ).status(),
          200,
        );
        await goto(fresh, invite);
    ...
    await check(
      "roster and message page two show exact DB rows without overlap",
      async () => {
        for (let i = 0; i < 23; i++) {
          await db.query(
            "INSERT INTO guests(event_id,edit_token_hash,name,status,marker,created_at) VALUES($1,$2,$3,'going','rental',now()+$4*interval '1 second')",
            [event.id, hashToken(token()), `${prefix}-roster-${i}`, i],
          );
          await db.query(
            "INSERT INTO messages(event_id,user_id,body,created_at) VALUES($1,$2,$3,now()+$4*interval '1 second')",
            [event.id, memberUser.id, `${prefix}-message-${i}`, i],
          );
        }
        for (const kind of ["players", "messages"]) {
          ...
          const rows = (
            await db.query(
              roster
                ? "SELECT name AS value FROM guests WHERE event_id=$1 AND approval='approved' ORDER BY created_at,id"
                : "SELECT body AS value FROM messages WHERE event_id=$1 ORDER BY created_at DESC,id",
              [event.id],
            )
          ).rows;
          await goto(owner, `${day}/${kind}`);
          const selector = roster
            ? ".day-roster .day-player"
            : ".day-message-list > article";
          assert.equal(await owner.locator(selector).count(), 20);
          const first = await owner.locator(selector).allTextContents();
          rows
            .slice(0, 20)
            .forEach((row, i) => assert.ok(first[i].includes(row.value)));
          await owner
            .locator(
              `a[href="${day}/${kind}?${roster ? "guestPage" : "messagePage"}=2"]`,
            )
            .click();
          ...
          rows
            .slice(20)
            .forEach((row, i) =>
              assert.ok(second[i].includes(row.value), `${kind} row ${i}`),
            );
    ...
    if (extension)
      await extension({
        ...
        image,
        diagnostics,
        screenshotDirectory: UI_SCREENSHOT_DIR,
      });
    if (screenshots) {
      ...
      for (const width of [1440, 1920, 390, 320]) {
        for (const [name, route, page] of [
          ["landing", "/", visitor],
          ["explore", "/explore", visitor],
          ["profile", "/profile", member],
          ["day", day, owner],
          ["players", `${day}/players`, owner],
          ["schedule", `${day}/schedule`, owner],
          ["teams", `${day}/teams`, owner],
          ["gear", `${day}/gear`, owner],
          ["messages", `${day}/messages`, owner],
          ...
          await check(`responsive ${name} ${width}px`, async () => {
            ...
            await page.screenshot({
              path: join(UI_SCREENSHOT_DIR!, `${name}-${width}.png`),
              fullPage: true,
            });
    ...
    await browser.close();
    const fixtureIds = (
      await db.query("SELECT id::text FROM users WHERE email=ANY($1::text[])", [
        emails,
      ])
    ).rows.map((row) => row.id);
    if (event) fixtureIds.push(event.id);
    await db.query(
      "DELETE FROM activity WHERE EXISTS (SELECT 1 FROM unnest($1::text[]) AS fixture(id) WHERE position(fixture.id in detail)>0)",
      [fixtureIds],
    );
    await db.query("DELETE FROM users WHERE email=ANY($1::text[])", [emails]);
    assert.equal(
      (await db.query("SELECT id FROM events WHERE title=$1", [prefix]))
        .rowCount,
      0,
    );
    await db.end();
      ...
      JSON.stringify(
        {
          ...
          failures,
          diagnostics,
          screenshotDirectory: screenshots ? UI_SCREENSHOT_DIR : undefined,
        },
        null,
        2,
      ),
